...
Meta-learning of lambda_unchosen (multiplication) with policy-gradient method
Contexts:
...
Critical parameters: epsilon, learning rate, unchosen multiplication 
...
def simulate_RW_MLuc_adversarial(x, Q_alpha, eps, reward_alpha, ML_alpha_mean, ML_alpha_std, T, Q_int, ML_mean_int, ML_std_int, scale, update, reward_stable, reward_volatile):
    ...
    for t in range(T):
        ...
        uc = scale*(np.exp(ML)/(1+np.exp(ML)))
        uc_mean = scale*(np.exp(ML_mean)/(1+np.exp(ML_mean)))
        uc_std = scale*(np.exp(ML_std)/(1+np.exp(ML_std)))
        uc_var[t] = uc
...
uc_mean_int = 1
MLuc_mean_int = 0 #null becomes 1 in the logit transformation #np.log(uc_mean_int/(1-uc_mean_int))
...
save_dir_first = '/Users/jareynde/OneDrive - UGent/1A_Main_Project/Models/1Simulations/Env_HideAndSeek/output/lambda_unchosen-mult-8choice'
